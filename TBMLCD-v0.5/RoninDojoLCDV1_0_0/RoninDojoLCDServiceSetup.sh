...
#-------------------------------------------------------------------------------
# This script is used to create an RoninDojo lcd service that starts at boot
# systemd is used.
...
while $gettingCurrency 
	do
		...
		if [ "$validationResult" = "Valid" ]; then
    			echo "Creating RoninDojo ST7735 LCD Service."
...
# Create A Unit File
echo "[Unit]
Description=RoninDojo LCD Service
After=multi-user.target
[Service]
Type=idle
ExecStart=/usr/bin/python3 $cwd/RoninDojoLCD.py $newCurrency $userScreenChoices
[Install]
WantedBy=multi-user.target" > /lib/systemd/system/RoninDojoST7735LCD.service

				# The permission on the unit file needs to be set to 644
				chmod 644 /lib/systemd/system/RoninDojoST7735LCD.service

				# Configure systemd
				systemctl daemon-reload
				systemctl enable RoninDojoST7735LCD.service

				# Start the service
				systemctl start RoninDojoST7735LCD.service
				echo "Done Creating RoninDojo ST7735 LCD Service."
		else
