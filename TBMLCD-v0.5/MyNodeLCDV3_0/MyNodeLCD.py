...
#-------------------------------------------------------------------------------
#-------------------------------------------------------------------------------
# This script displays eight screens on your MyNode Node:
#    1. First screen displays the MyNode logo.
...
def check_mynode_and_mempool():
    """Checks for local mempool app."""
        
    #  Try to ping mynode.local and check for installed apps
    mynode = False
    mempool = False
    config = load_config(True)
    config_file = os.path.join(basedir, 'config.ini')
    try:
        url = config['MYNODE']['url']
    except Exception:
    ...
    if mynode:
        ...
        try: 
            result = tor_request(url)
            if not isinstance(result, requests.models.Response):
                raise Exception('Did not get a return from http://mynode.local:4080/')
            if not result.ok:
...
print('Running mynode 1.8 Inch LCD script Version 2.0.1')

#Display mynode logo first for 60 seconds
display_background_image('mynode_logo.png')
...
# Initial check for mynode and mempool
mempool_status = check_mynode_and_mempool()
print(f"Local Mempool app status = {mempool_status}")
